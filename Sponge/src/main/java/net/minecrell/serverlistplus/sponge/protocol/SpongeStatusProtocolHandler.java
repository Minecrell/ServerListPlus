...
final class SpongeStatusProtocolHandler implements StatusProtocolHandler {
    ...
    SpongeStatusProtocolHandler(Logger logger) {
        logger.debug("Platform protocol version: {}", Sponge.platform().minecraftVersion().protocolVersion());
    ...
    private static OptionalInt getProtocolVersion(MinecraftVersion version) {
        return OptionalInt.of(version.protocolVersion());
    }
    ...
    public void setVersion(ClientPingServerEvent.Response ping, net.minecrell.serverlistplus.core.status.StatusResponse response) {
        ...
        if (version != null) {
            ping.version().setName(version);
        }

        Integer protocol = response.getProtocolVersion();
        if (protocol != null) {
            ping.version().setProtocolVersion(protocol);
        }
    }
}
